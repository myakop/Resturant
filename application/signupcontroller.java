...
public class signupcontroller implements Initializable{
	...
	public void initialize(URL location, ResourceBundle resources) { // fill the comboBox in genders and neighborhood
	...
	public void submit (ActionEvent event)throws IOException{
		...
		for(Customer c : Main.res.getCustomers().values()) {
			...
			check=false; // check the username
		}
		}
		...
		checkid(c);
		if(check) {
		if(Main.res.addCustomer(c)) { //sign up and  add customer 
			Main.update();
	...
	public void checkid (Customer c) {
		int x=0;
