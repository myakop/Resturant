...
public class EditDetailsCustController implements Initializable {
	@FXML
	...
	public void initialize(URL location, ResourceBundle resources) { 
		...
		firstname.setText(Main.cust.getFirstName());
		lastname.setText(Main.cust.getLastName());
		username.setText(Main.cust.getUsername());
		gend.setValue(Main.cust.getGender().toString()); // fill the elements in the details customer that we need to edit
	...
	for(Customer c : Main.res.getCustomers().values()) {
		if(c.getUsername().equals(username.getText())  && !Main.cust.getUsername().equals(username.getText())){
			checkusername=false; //check username
		...
		if(checkusername) {
			...
			Main.cust.setGender(Gender.valueOf(gend.getValue())); // set the new details
